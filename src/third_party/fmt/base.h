...
template <typename T> auto convert_for_visit(T) -> monostate
{
    return {};
}
#endif
...
template <typename Int> FMT_CONSTEXPR auto to_unsigned(Int value) -> make_unsigned_t<Int>
{
...
struct is_std_string_like<
    T,
    void_t<decltype(std::declval<T>().find_first_of(typename T::value_type(), 0))>>
...
struct is_back_insert_iterator<
    It,
    bool_constant<
        std::is_same<decltype(adl::invoke_back_inserter<typename It::container_type>()),
...
enum {
    ...
    uint_set =
        set(type::uint_type) | set(type::ulong_long_type) | set(type::uint128_type),
    bool_set = set(type::bool_type),
    char_set = set(type::char_type),
    float_set =
        set(type::float_type) | set(type::double_type) | set(type::long_double_type),
...
FMT_DEPRECATED FMT_NORETURN inline void throw_format_error(const char* message)
{
...
public:
    explicit FMT_CONSTEXPR compile_parse_context(basic_string_view<Char> format_str,
                                                 int                     num_args,
                                                 const type*             types,
                                                 int                     next_arg_id = 0)
...
protected:
    ...
    constexpr buffer(grow_fun grow,
                     T*       p   = nullptr,
                     size_t   sz  = 0,
...
public:
    ...
    template <typename Idx> FMT_CONSTEXPR auto operator[](Idx index) const -> const T&
    {
        return ptr_[index];
    }
};
...
public:
    ...
    auto operator++(int) -> basic_appender
    {
        return *this;
    }
};
...
namespace detail {
template <typename T> struct is_back_insert_iterator<basic_appender<T>> : std::true_type {
};

template <typename T, typename Enable = void> struct locking : std::true_type {
};
template <typename T>
...
constexpr auto has_const_formatter_impl(T*)
    -> decltype(typename Context::template formatter_type<T>()
                    .format(std::declval<const T&>(), std::declval<Context&>()),
                true)
{
    return true;
}
template <typename Context> constexpr auto has_const_formatter_impl(...) -> bool
{
    return false;
}
...
struct is_buffer_appender<
    It,
    bool_constant<is_back_insert_iterator<It>::value &&
                  std::is_base_of<buffer<typename It::container_type::value_type>,
                                  typename It::container_type>::value>> : std::true_type {
};
...
template <bool B = false> constexpr auto count() -> size_t
{
    return B ? 1 : 0;
}
...
public:
    ...
    constexpr FMT_ALWAYS_INLINE value()
        : no_value()
    {
    }
    constexpr FMT_ALWAYS_INLINE value(int val)
        : int_value(val)
    {
    }
    constexpr FMT_ALWAYS_INLINE value(unsigned val)
        : uint_value(val)
    {
    }
    constexpr FMT_ALWAYS_INLINE value(long long val)
        : long_long_value(val)
    {
    }
    ...
    constexpr FMT_ALWAYS_INLINE value(float val)
        : float_value(val)
    {
    }
    constexpr FMT_ALWAYS_INLINE value(double val)
        : double_value(val)
    {
    }
    ...
    constexpr FMT_ALWAYS_INLINE value(bool val)
        : bool_value(val)
    {
    }
    constexpr FMT_ALWAYS_INLINE value(char_type val)
        : char_value(val)
    {
    }
...
template <typename Context> struct arg_mapper {
    ...
    FMT_MAP_API auto
    map(const T& named_arg) -> decltype(FMT_DECLTYPE_THIS map(named_arg.value))
    {
...
FMT_CONSTEXPR auto make_arg(T& val) -> value<Context>
{
    ...
    enum { formattable_char = !std::is_same<arg_type, unformattable_char>::value };
    static_assert(formattable_char, "Mixing character types is disallowed.");

    // Formatting of arbitrary pointers is disallowed. If you want to format a
    // pointer cast it to `void*` or `const void*`. In particular, this forbids
    // formatting of `[const] volatile char*` printed as bool by iostreams.
    enum { formattable_pointer = !std::is_same<arg_type, unformattable_pointer>::value };
...
using arg_t =
    conditional_t<NUM_ARGS <= max_packed_args, value<Context>, basic_format_arg<Context>>;
...
template <typename Char, typename T, FMT_ENABLE_IF(is_named_arg<T>::value)>
void init_named_arg(named_arg_info<Char>* named_args,
                    int&                  arg_index,
                    int&                  named_arg_index,
                    const T&              arg)
{
...
struct format_arg_store {
    ...
    }
    {
        ...
        (void)dummy{
            0, (init_named_arg(named_args, arg_index, named_arg_index, values), 0)...};
    }

    format_arg_store(format_arg_store&& rhs)
    {
        args[0] = {named_args, NUM_NAMED_ARGS};
        for (size_t i = 1; i < ARGS_ARR_SIZE; ++i)
            args[i] = rhs.args[i];
...
public:
    ...
    auto format_custom(const char_type*                      parse_begin,
                       ...
                       Context&                              ctx) -> bool
    {
        if (type_ != detail::type::custom_type)
            return false;
...
FMT_DEPRECATED FMT_CONSTEXPR auto
...
public:
    ...
    constexpr FMT_ALWAYS_INLINE basic_format_args(
        const detail::format_arg_store<Context, NUM_ARGS, NUM_NAMED_ARGS, DESC>& store)
        : desc_(DESC)
        , values_(store.args + (NUM_NAMED_ARGS != 0 ? 1 : 0))
    {
    }
    ...
    constexpr basic_format_args(
        const detail::format_arg_store<Context, NUM_ARGS, NUM_NAMED_ARGS, DESC>& store)
        : desc_(DESC)
        , args_(store.args + (NUM_NAMED_ARGS != 0 ? 1 : 0))
    {
    }
    ...
    FMT_CONSTEXPR auto get(int id) const -> format_arg
    {
        ...
        if (static_cast<unsigned>(id) >= detail::max_packed_args)
            return arg;
        arg.type_ = type(id);
...
public:
    ...
    FMT_CONSTEXPR context(iterator                   out,
...
using basic_format_context = conditional_t<std::is_same<OutputIt, appender>::value,
                                           context,
...
using is_formattable =
    bool_constant<!std::is_base_of<detail::unformattable,
                                   decltype(detail::arg_mapper<buffered_context<Char>>()
...
constexpr FMT_ALWAYS_INLINE auto
make_format_args(T&... args) -> detail::format_arg_store<Context, NUM_ARGS, 0, DESC>
{
    return {{detail::make_arg<NUM_ARGS <= detail::max_packed_args, Context>(args)...}};
}
...
public:
    template <typename Char> FMT_CONSTEXPR void operator=(basic_string_view<Char> s)
    {
        auto size = s.size();
...
template <>
inline auto find<false, char>(const char*  first,
                              const char*  last,
                              char         value,
                              const char*& out) -> bool
{
    out = static_cast<const char*>(memchr(first, value, to_unsigned(last - first)));
    return out != nullptr;
}
...
FMT_CONSTEXPR auto
do_parse_arg_id(const Char* begin, const Char* end, Handler&& handler) -> const Char*
{
    Char c = *begin;
    if (c >= '0' && c <= '9') {
        int index = 0;
...
template <typename Char, typename Handler>
FMT_CONSTEXPR auto
parse_arg_id(const Char* begin, const Char* end, Handler&& handler) -> const Char*
{
...
template <typename Char>
FMT_CONSTEXPR auto
parse_dynamic_spec(const Char*                       begin,
                   const Char*                       end,
                   int&                              value,
                   arg_ref<Char>&                    ref,
                   basic_format_parse_context<Char>& ctx) -> const Char*
{
    ...
    if ('0' <= *begin && *begin <= '9') {
        ...
        if (begin != end)
            begin = parse_arg_id(begin, end, handler);
        if (begin != end && *begin == '}')
            return ++begin;
        report_error("invalid format string");
    }
...
template <typename Char>
FMT_CONSTEXPR auto parse_format_specs(const Char*                       begin,
                                      const Char*                       end,
                                      ...
                                      type arg_type) -> const Char*
{
    ...
    for (;;) {
        ...
        default: {
            ...
            specs.fill  = basic_string_view<Char>(begin, to_unsigned(fill_end - begin));
            specs.align = align;
            begin       = fill_end + 1;
        }
        }
...
public:
    ...
    FMT_CONSTEXPR auto
    on_format_specs(int id, const Char* begin, const Char*) -> const Char*
    {
...
template <typename S> using is_compile_string = std::is_base_of<compile_string, S>;
...
template <
    typename OutputIt,
    FMT_ENABLE_IF(detail::is_output_iterator<remove_cvref_t<OutputIt>, char>::value)>
...
template <typename OutputIt, typename Sentinel = OutputIt> struct format_to_result {
...
FMT_INLINE auto
format_to(char (&out)[N], format_string<T...> fmt, T&&... args) -> format_to_result<char*>
{
