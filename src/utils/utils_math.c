 ...
 * @todo..  min should be less than max
 */
...
/**
 * @brief   return n numbers in range [min, max]
 * @note    The numbers are not guaranteed to be unique.
 */
void utils_rand_n(int* dst, int min, int max, int n)
{
    for (int i = 0; i < n; i++) {
        dst[i] = utils_rand(min, max);
    }

    utils_shell_sort(dst, n);
}

/**
 * @brief   Split a range into n segments with minimum value in each segment
 */
void utils_rand_split(int* dst, int range_min, int range_max, int n, int min_value)
{
    if (n == 1) {
        dst[0] = range_max - range_min; /* If n is 1, the whole range is the only segment */
        return;
    }

    range_max -= n * min_value; /* Adjust the max range to account for minimum value in each segment */

    int* rand_points = (int*)malloc((n - 1) * sizeof(int)); /* split to n need n-1 points */
    utils_rand_n(rand_points, range_min, range_max, n - 1);

    for (int i = 0; i < n; i++) {
        if (i == 0) {
            dst[i] = rand_points[i] - range_min + min_value; /* First segment from min to first random point */
        }
        else if (i == n - 1) {
            dst[i] = range_max - rand_points[i - 1] + min_value; /* Last segment from last random point to max */
        }
        else {
            dst[i] = rand_points[i] - rand_points[i - 1] + min_value; /* Middle segments between random points */
        }
    }
...
{
    if (num >= 0) {
        return (int)(num + 0.9999999999); /* 向上取整 */
    }
    else {
        return (int)(num - 0.9999999999); /* 向下取整 */
    }
}
